...
export default function Contracts({ data, persist, live }) {
  const { sampleContracts, vendors } = data
    ...
    <div className="fade-in">
      ...
      <div className="grid" style={{ gridTemplateColumns: '0.85fr 1.15fr', gap: 20 }}>
        <div className="card pad-lg">
          ...
          <div className="faint" style={{ fontSize: 12.5, marginBottom: 10 }}>Try an example:</div>
          <div className="stack">
            ...
            <div className="stack fade-in">
                ...
                <div className="extract-grid mt-sm">
                  ...
                  <div>
                    ...
                    <div className="kv">
                      ...
                      <span className="v"><span className={`badge ${result.gratuityIncluded === false ? 'high' : result.gratuityIncluded === true ? 'ok' : 'med'}`}>{String(result.gratuityIncluded)}</span></span>
                    </div>
                    <div className="kv">
                ...
                {(result.keyDates || []).length > 0 && (
                  <>
                    ...
                    <div className="row wrap gap-sm">
                      {result.keyDates.map((k, i) => (<span className="tag" key={i}>{k.label}: <b style={{ color: 'var(--ink)' }}>{k.date}</b></span>))}
                    </div>
                  </>
                )}
              </div>
              ...
              {(result.watchOuts || []).length > 0 && (
                <div className="card pad-lg">
                  ...
                  {result.watchOuts.map((w, i) => (<div className="watchout" key={i}><span style={{ color: 'var(--amber)' }}>&bull;</span> {w}</div>))}
                </div>
              )}
